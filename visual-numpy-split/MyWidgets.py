...
class MainWindow(QMainWindow):
    ...
    def __init__(self, parent=None):
        ...
        if (F):
           self.view = MyView(self)
           model = MyModel(self.view)
           delegate = MyDelegate(self.view)
           self.view.setModel(model)
           self.view.setItemDelegate(delegate)

           self.view2 = MyView(self)
           self.view2.setModel(model)
           self.view2.setItemDelegate(delegate)
    ...
    def mergeCells(self):
        ...
        topLeftIndex = view.model().index(min(rows), min(columns))
        bottomRightIndex = view.model().index(max(rows), max(columns))
    ...
    def bc_setSpan( self, source, row,  column,  rowSpanCount,  columnSpanCount) :
        if source == self.view:
            self.view2.bc_setSpan( row,  column,  rowSpanCount,  columnSpanCount) 
        elif source == self.view2:
            self.view.bc_setSpan( row,  column,  rowSpanCount,  columnSpanCount) 

    def mergeCells_(self):
        """Basic merge cell funcionality"""
        selectionModel = self.view.selectionModel()
        selectedIndexes = selectionModel.selectedIndexes()
        rows = []
